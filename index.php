...
<head>
    ...
    <link rel="preload" href="assets/fonts/uicons-regular-rounded.woff2" as="font" type="font/woff2" crossorigin>
    <link rel="stylesheet" href="assets/css/uicon.css?ver=1.0.2">
    <link rel='stylesheet' href='assets/css/8content.css?ver=1.0.7' type='text/css' media='all' />
    <title><?php echo $title; ?></title>
    <link rel="icon" href="<?php echo $siteIcon; ?>" />
    <meta name="title" content="<?php echo $title; ?>">
    <meta name="description" content="<?php echo $metaDesc ?>">
    <meta property="og:type" content="website">
    <meta property="og:url" content="https://password-generator.8content.com/">
    <meta property="og:title" content="<?php echo $title; ?>">
    <meta property="og:description" content="<?php echo $metaDesc ?>">
    <meta property="og:image" content="<?php echo $siteIcon; ?>">
    <meta property="twitter:card" content="summary_large_image">
    <meta property="twitter:url" content="https://password-generator.8content.com/">
    <meta property="twitter:title" content="<?php echo $title; ?>">
    <meta property="twitter:description" content="<?php echo $metaDesc ?>">
    <meta property="twitter:image" content="<?php echo $siteIcon; ?>">
</head>

<body>
    <div class="app" id="app">
        <div class="section">
            <div class="row navigation-menu align-items-center">
                ...
                <div class="menu-col-right d-flex justify-content-end">
                    <i class="fi fi-rr-align-right" onclick="openNav()"></i>
                </div>
            </div>
        </div>
        <div class="section main-section">
            <div class="row main-content">
                <div class="col-12">
                    <h1 class="text-center mb-0 main-h1-title">สร้างรหัสผ่านปลอดภัย สุ่มรหัสผ่านออนไลน์</h1>
                    <p class="text-center" style="margin-top: 0.25em; margin-bottom: 2.25em;">สร้างรหัสผ่านแบบปลอดภัย รหัสผ่านแบบยาก สุ่มรหัสผ่าน สุ่มรหัสผ่านออนไลน์</p>
                    <div class="form-group form-group-2-col row align-items-center">
                        <div class="col-8">
                            <input type="text" class="form-control" id="password-text">
                        </div>
                        <div class="col-4 d-flex justify-content-end gap-1 justify-content-mb-center">
                            <button class="btn btn-w-icon" id="copy-password">
                                <i class="fi fi-rr-copy-alt"></i>
                                <span class="show-mb">คัดลอก</span>
                            </button>
                            <button class="btn btn-w-icon" id="regen-password">
                                <i class="fi fi-rr-rotate-right"></i>
                                <span class="show-mb">สร้างใหม่</span>
                            </button>
                        </div>
                    </div>
                    <div class="form-group form-group-2-col row align-items-center mt-2" style="background: #fff;">
                        <div class="col-6 col-mob-no-change">
                            <h4 class="">ความยาวของรหัสผ่าน</h4>
                        </div>
                        <div class="col-6 d-flex justify-content-end gap-1 col-mob-no-change">
                            <h4 class=""><span id="demo"></span> ตัวอักษร</h4>
                        </div>
                        <div class="col-12">
                            <input type="range" min="1" max="30" value="15" class="slider" id="myRange">
                        </div>
                        <div class="row mt-4 checkbox-group" style="width: 100%; margin-top: 2em;">
                            <div class="col-3 col-mb-6">
                                <label class="container"> Uppercase
                                    <input type="checkbox" checked="checked" id="uppercase" class="customize-checkbox" value="u">
                                    <span class="checkmark"></span>
                                </label>
                            </div>
                            <div class="col-3 col-mb-6">
                                <label class="container"> Lowercase
                                    <input type="checkbox" checked="checked" id="lowercase" class="customize-checkbox" value="l">
                                    <span class="checkmark"></span>
                                </label>
                            </div>
                            <div class="col-3 col-mb-6">
                                <label class="container"> Numbers
                                    <input type="checkbox" checked="checked" id="numbers" class="customize-checkbox" value="n">
                                    <span class="checkmark"></span>
                                </label>
                            </div>
                            <div class="col-3 col-mb-6">
                                <label class="container"> Symbols
                                    <input type="checkbox" checked="checked" id="symbols" class="customize-checkbox" value="s">
                                    <span class="checkmark"></span>
                                </label>
                            </div>
                        </div>
    ...
    <script type="text/javascript">
        function create_password(length, char) {
            var result = '';
            var characters = '';
            if (char.includes('u')) {
        ...
        var notify = '<div class="notify-warpper"> <div class="notify-content"> คัดลอกรหัสผ่านสำเร็จ </div> </div>';
        var slider = document.getElementById("myRange");
        var output = document.getElementById("demo");
        var checkbox_group = document.querySelectorAll('.customize-checkbox');
        var pw_arr = [];
        output.innerHTML = slider.value;
        document.getElementById("password-text").value = create_password(15, ['u', 'l', 'n', 's']);
        document.getElementById("copy-password").onclick = function() {
            var copyText = document.getElementById("password-text");
            copyText.select();
            copyText.setSelectionRange(0, 99999);
            navigator.clipboard.writeText(copyText.value);
            document.querySelector('.app').insertAdjacentHTML('afterend', notify);
        };
        document.querySelectorAll('.customize-checkbox:checked').forEach((checkbox) => {
            pw_arr.push(checkbox.value);
        });
        checkbox_group.forEach((checkbox) => {
            ...
            checkbox.addEventListener('click', (event) => {
                ...
                if (event.target.checked) {
                    pw_arr.push(event.target.value);
                } else {
                    pw_arr = pw_arr.filter(e => e !== event.target.value);
                }
                document.getElementById("password-text").value = create_password(parseInt(output.innerHTML), pw_arr);
            });
        });
        slider.oninput = function() {
            output.innerHTML = this.value;
            document.getElementById("password-text").value = create_password(this.value, pw_arr);
        };
        document.getElementById("regen-password").onclick = function() {
            document.getElementById("password-text").value = create_password(slider.value, pw_arr);
        };
    ...
    <!-- Google tag (gtag.js) -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=G-3TYM52HK06"></script>
    <script>
    window.dataLayer = window.dataLayer || [];
    function gtag(){dataLayer.push(arguments);}
    gtag('js', new Date());

    gtag('config', 'G-3TYM52HK06');
    </script>
</body>
